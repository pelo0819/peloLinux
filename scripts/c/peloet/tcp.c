		...
		if(TcpTable[i].myPort!=0)
        {
			if(TcpTable[i].status==TCP_ESTABLISHED)
            {
				printf(
                    ...
                    TcpTable[i].myPort,
                    inet_ntop(AF_INET, &TcpTable[i].dstAddr, buf2, sizeof(buf2)),
                    TcpTable[i].dstPort,
...
int TcpSocketClose(u_int16_t port)
{
    ...
    if(no == -1)
    {
        printf("TcpSocketClose:%u:not exitsts\n", port);
        return -1;
    }
    pthread_rwlock_wrlock(&TcpTableLock);
...
int TcpSend(int soc,u_int16_t sport,u_int8_t *data,int len)
{
	...
	printf(
        "TcpSend:una=%u,nextSeq=%u:success\n",
        TcpTable[no].snd.una-TcpTable[no].snd.iss,
        TcpTable[no].snd.nxt-TcpTable[no].snd.iss
    );
...
)
{
    ...
    {
        // rcv.nxt == 0なら初期受信、rcv.nxt == ntols(tcp->seq)なら受信済みなので受信処理しない
        ...
        {
            switch (TcpTable[no].status)
            {
                // 3ウェイハンドシェイクで[SYN]を送信した後、[SYN, ACK]を待っている状態
                case TCP_SYN_SENT:
                    // resetフラグが立っている場合はsocketを閉じる
                    if(tcp->rst == 1)
                    {
                ...
                case TCP_LISTEN:
                    if(tcp->syn == 1)
                    {
					    ...
					    TcpTable[no].rcv.nxt=ntohl(tcp->seq)+1;
					    TcpSendSyn(soc,no,1); // SYN/ACKを送り返す
                    }
                    break;
                case TCP_FIN_WAIT1:// 接続解除メッセージを送信したらこのステートになる
                    if(tcp->rst == 1)
                    {
                        printf("TcpRecv:%d:SYN_SENT:rst\n", no);
                        TcpTable[no].status = TCP_CLOSE;
